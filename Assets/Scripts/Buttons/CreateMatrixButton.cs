...
public class CreateMatrixButton : MonoBehaviour
{
    //
    [SerializeField] Sprite changeImage;
    int pointsCost = 0;
    public bool buttonPressed = false;
    public float decreaseResearchPotentialCoefficient = 1;
        ...
        if (World.Points >= pointsCost)
        {
            if (!buttonPressed) // button can be pressed only one time 
            {
                ...
                if (countries.Length > 0)
                {
                    ...
                    {
                        country.GetComponent<Country>().ChangeDecreaseResearchPotentialCoefficient(decreaseResearchPotentialCoefficient);
                    }
                }
                World.ChngeMatrixCreationFlag(true);
